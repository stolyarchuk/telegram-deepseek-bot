...
func CreateBot() *tgbotapi.BotAPI {
	...
	if err := RegisterBotCommands(conf.Bot); err != nil {
		logger.Warn("Failed to register commands during bot creation", "err", err)
	}

	return conf.Bot
}

// RegisterBotCommands registers all available bot commands with descriptions using global scope.
// This function registers commands that are visible to all users in all chat types (private, group, supergroup).
// Commands are registered with internationalization support and exclude admin-only commands.
func RegisterBotCommands(bot *tgbotapi.BotAPI) error {
	// User commands extracted from README.md (excluding admin commands)
	...
	// Add MCP command if tools are enabled
	if *conf.UseTools {
	...
	// Create SetMyCommands config with explicit global scope
	// This ensures commands are available to all users in all chat types
	cmdCfg := tgbotapi.SetMyCommandsConfig{
		Commands: commands,
		Scope: &tgbotapi.BotCommandScope{
			Type: "default", // Global scope for all chats and users
		},
		LanguageCode: "", // Empty for default language
	}

	_, err := bot.Request(cmdCfg)
	if err != nil {
		logger.Warn("Failed to register bot commands globally", "err", err)
		return err
	}

	logger.Info("Successfully registered bot commands globally", "count", len(commands))
	return nil
}
